# SAGU → SUBSISTENCE (рабочий репозиторий проекта)

Это официальный дом проекта **Subsistence** — Unity-хоррор-выживача «механики Rust в мире Backrooms».
Проект передан из другой песочницы (архив `Subsistence_1.0.0-alpha.zip`, 470 файлов) и распакован сюда целиком.

## Как устроен этот репозиторий

| Путь | Что это |
|---|---|
| `Assets/ Packages/ ProjectSettings/` | Unity-проект 2022.3.62f2 + HDRP (открывать в Unity Hub корень репо) |
| `docs/` | ГДД, ответы опросников (ANSWERS.md, ANSWERS_V2.md), роадмап, ассет-байбл, превью-листы моделей |
| `tools/` | Blender-пайплайн моделей, линтеры C#, сервер опросника |
| `site/` | Веб-страницы: опросники (questions.html v1, questions2.html v2, **questions3.html v3 — 40 вопросов от новой ИИ**) |
| `.github/workflows/` | Воркфлоу: `fetch-archive.yml` (скачать архив с гугл-диска → в репо), `fetch-sysroot.yml` (системные библиотеки для headless-Blender) |
| `incoming/` | Служебные артефакты: `sysroot-bookworm.tar.gz` (библиотеки для bpy) |
| `answers/` (gitignored) | Ответы опросников, собранные сервером `tools/answers_server.mjs` |

Исходный zip лежит в истории git (`git log --all -- incoming/`) и на гугл-диске; в рабочей копии его нет,
чтобы не тащить 32 МБ дважды.

## Окружение песочницы Arena (для ИИ)

- Сеть песочницы видит только `github.com` / `codeload.github.com` / `pypi.org` / `registry.npmjs.org`.
  Всё остальное (гугл-диск, blender.org, deb.debian.org) качается через GitHub Actions — воркфлоу выше.
- **Blender**: ставится как python-модуль `bpy` (5.0.1) + sysroot-библиотеки. Восстановление после
  рестарта песочницы: `bash tools/setup_blender.sh`. Проверка рендера — `docs/previews/_render_sandbox_test.png`.
- Сборка ответов с сайта-опросника: `node tools/answers_server.mjs` (порт 3000), ответы падают в `answers/`.

## Быстрый старт игры — см. ЧИТАЙ_МЕНЯ.txt и раздел ниже
